...
# Step 1: Enable EventBridge notifications on S3 bucket
Write-Host "`nStep 1: Enabling EventBridge for S3 bucket..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -eq 0) {
    Write-Host "success EventBridge enabled on S3 bucket" -ForegroundColor Green
} else {
    Write-Host "x Failed to enable EventBridge" -ForegroundColor Red
}
...
aws events put-rule `
  ...
  --event-pattern '{\"source\":[\"aws.s3\"],\"detail-type\":[\"Object Created\"],\"detail\":{\"bucket\":{\"name\":[\"tcss562-suzzanne-data\"]},\"object\":{\"key\":[{\"suffix\":\".csv\"},{\"anything-but\":{\"suffix\":\".transformed.csv\"}}]}}}' `
  --description "Trigger TransformCSV when CSV uploaded to S3" `
  --region us-west-2

if ($LASTEXITCODE -eq 0) {
    Write-Host "success EventBridge rule created: S3-CSV-Upload-Trigger-Transform" -ForegroundColor Green
} else {
    Write-Host "x Failed to create EventBridge rule" -ForegroundColor Red
}
...
Write-Host "success Lambda permission added for Transform" -ForegroundColor Green
...
if ($LASTEXITCODE -eq 0) {
    Write-Host "success Transform Lambda connected to EventBridge" -ForegroundColor Green
} else {
    Write-Host "x Failed to add target" -ForegroundColor Red
}
...
aws events put-rule `
  ...
  --event-pattern '{\"source\":[\"aws.s3\"],\"detail-type\":[\"Object Created\"],\"detail\":{\"bucket\":{\"name\":[\"tcss562-suzzanne-data\"]},\"object\":{\"key\":[{\"suffix\":\".transformed.csv\"}]}}}' `
  --description "Trigger CreateSQLiteDB when transformed CSV uploaded" `
  --region us-west-2

if ($LASTEXITCODE -eq 0) {
    Write-Host "success EventBridge rule created: S3-Transformed-CSV-Trigger-Load" -ForegroundColor Green
} else {
    Write-Host "x Failed to create EventBridge rule" -ForegroundColor Red
}
...
Write-Host "success Lambda permission added for Load" -ForegroundColor Green
...
if ($LASTEXITCODE -eq 0) {
    Write-Host "success Load Lambda connected to EventBridge" -ForegroundColor Green
} else {
    Write-Host "x Failed to add target" -ForegroundColor Red
}
